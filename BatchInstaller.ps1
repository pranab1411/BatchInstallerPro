...
.SYNOPSIS
    ...
    A modern, high-performance silent installer deployment tool for Windows with system sound notifications.
...
# ---------------------------------------------------------------------------
# Windows Notification Sound Engine
# ---------------------------------------------------------------------------
function Play-NotificationSound([string]$type) {
    try {
        if ($type -eq "SUCCESS") {
...
$chkSound.Text = "Sound Alerts"
$chkSound.Checked = $true
$chkSound.Location = New-Object System.Drawing.Point(235, 7)
$chkSound.AutoSize = $true
$chkSound.ForeColor = [System.Drawing.Color]::FromArgb(0, 230, 150)
$chkSound.Font = New-Object System.Drawing.Font("Segoe UI Semibold", 9)
$pnlToolBar.Controls.Add($chkSound)
...
$btnInstall.Add_Click({
    ...
    # Play Windows Notification Sounds
    if ($chkSound.Checked) {
        if ($failCount -eq 0) {
